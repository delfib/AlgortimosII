...
/**
 * This class HillClimbingEngine implements a Hill Climbing search strategy which can be used 
 * with any instance of StateInformed.
 */
public class HillClimbingEngine <S extends StateInformed, P extends StateProblem<S>> implements Engine<S,P> {
    ...
    // Constructor
    public HillClimbingEngine(){
        sp = null;
        path = new LinkedList<S>();
    }

    // Another Constructor
    public HillClimbingEngine(P sp){
        this.sp = sp;
        path = new LinkedList<S>();
    }

    /** 
    * Starts the search for successful states, following a hill climbing strategy.
    ...
    */
    public S performSearch(){
        ...
        while (!found) {
            ...
            if (current.isSuccess()){
                ...
                current = getFirstBestThan(current, succs);
                if (current == null){
                    break;
                }
            }
    ...
    /**
     * Private method that given a list of states, returns the first state that's better than the current one.
     * Return the first state that has a better valoration that the current.
     ...
     */
    public S getFirstBestThan(S current, List<S> succs){
        for (int i = 0; i < succs.size(); i++){
            if (succs.get(i).value() <= current.value()){
                return succs.get(i);
            }
        }
        return null;
